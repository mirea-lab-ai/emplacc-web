    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0">
        ...
        <div className="space-y-3">
          ...
          {isLoading && <div className="text-app-2 text-center py-8 animate-pulse">Загрузка…</div>}
          <div className="space-y-2">
            {filtered.map(p => (
              <div key={p.id}
                className={`t-surface rounded-2xl p-4 flex items-center gap-3 ring-1 cursor-pointer transition-all ${activeProblemId === p.id ? 'ring-emerald-500/40 bg-emerald-500/5' : 'ring-app hover:ring-white/15'}`}
                onClick={() => setActiveProblemId(activeProblemId === p.id ? null : p.id)}>
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-app truncate">{p.name}</div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <Link href={`/forum?problem=${p.id}`} onClick={e => e.stopPropagation()}
                    className="p-2 rounded-xl text-app-2 hover:text-app hover:bg-app-hover transition-colors" title="Открыть на форуме">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
                      <path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/>
                    </svg>
                  </Link>
                  <button onClick={e => { e.stopPropagation(); void handleDeleteProblem(p); }}
                    className="p-2 rounded-xl text-app-2 hover:text-red-400 hover:bg-red-500/10 transition-colors" title="Удалить тему">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
        ...
        {/* Messages panel */}
        <div className="t-surface rounded-2xl p-4 space-y-3 ring-1 ring-app min-h-[400px] max-h-[70vh] flex flex-col">
          {!activeProblem ? (
            <div className="flex-1 flex items-center justify-center text-app-3 text-sm">
              Выберите тему для просмотра сообщений
            </div>
          ) : (
            <>
              <div className="font-semibold text-app shrink-0">{activeProblem.name}</div>
              <div className="flex-1 overflow-y-auto space-y-2">
                {messages.length === 0 && <p className="text-app-3 text-sm text-center py-8">Сообщений нет</p>}
                {messages.map(m => (
                  <div key={m.id} className="flex items-start gap-3 p-3 rounded-xl bg-app-subtle hover:bg-app-hover group transition-colors">
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium text-emerald-300">{m.authorName || m.authorId}</div>
                      <p className="text-sm text-app-2 mt-0.5 line-clamp-3 break-words">{m.content}</p>
                      {m.createdAt && <div className="text-[10px] text-app-3 mt-1">{formatDateTime(m.createdAt)}</div>}
                    </div>
                    <button onClick={() => void handleDeleteMessage(m.id)}
                      className="p-1.5 rounded-lg text-app-3 hover:text-red-400 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all shrink-0" title="Удалить">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
                        <path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2m3 0l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/>
                      </svg>
                    </button>
