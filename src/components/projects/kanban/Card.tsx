...
export default function Card({
                                 task, fromColId, onRemove, isDeleting = false,
                             }: { task: UITask; fromColId: string; onRemove: () => void; isDeleting?: boolean }) {
    ...
    return (
        ...
        >
            <button
                onClick={onRemove}
                disabled={isDeleting}
                className="absolute top-2 right-2 hidden group-hover:inline-flex rounded-md p-1 ring-1 ring-white/10 hover:bg-[#ef4657]/25 hover:text-white hover:ring-[#ef4657]/40 transition disabled:opacity-50 disabled:cursor-not-allowed"
